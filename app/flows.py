...
def _truncate_body(b: bytes, content_type: str = "") -> tuple[str, bool]:
    """按字节截到 MAX_BODY_BYTES，再按 content-type 决定文本/二进制展示。返回 (text, truncated)。"""
    if not b:
        return "", False
    truncated = len(b) > MAX_BODY_BYTES
    raw_len = len(b)
    if truncated:
        b = b[:MAX_BODY_BYTES]
    # content-type 明确不是文本时，给个友好占位，避免一片乱码
    if content_type and not _TEXTUAL_RE.match(content_type.strip()):
        return f"(二进制内容: {content_type}, {raw_len} bytes)", truncated
    try:
        return b.decode("utf-8", errors="replace"), truncated
    except Exception:
        return repr(b), truncated
